  ...
  switch (code) {
    case '404':
      throw new Response('Not Found', {
        ...
        headers: { 'X-Route-Timing': (performance.now() - start).toFixed(2) }
      });
...
export default function ErrorCode({ params }: Route.ComponentProps) {
  return (
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ marginTop: '1rem' }}>{statusText}</h2>
      ...
      <div style={{
        ...
        <h3 style={{ marginTop: 0 }}>ℹ️ About this error:</h3>
        <ul style={{ marginBottom: 0 }}>
          <li><strong>Status:</strong> {statusCode}</li>
          <li><strong>Type:</strong> {error instanceof Response ? 'HTTP Response Error' : 'Runtime Error'}</li>
          <li><strong>Caught by:</strong> Route ErrorBoundary in <code>errors.$code.tsx</code></li>
          <li><strong>SSR:</strong> This page was server-rendered with the error</li>
        </ul>
      </div>

      <div style={{ marginTop: '2rem' }}>
        <Link to="/errors" style={{ color: '#0066cc', textDecoration: 'none' }}>
          ← Back to Error Demo
        </Link>
        {' | '}
