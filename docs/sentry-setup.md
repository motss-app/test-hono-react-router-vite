...
- keeps `enableRpcTracePropagation` so RPC receivers continue incoming traces, and scopes caller-side propagation to Sentry-instrumented bindings via `rpcTracePropagationBindings` (`['FRONTEND', 'BFF']` on the gateway, `[]` elsewhere) so non-Sentry receivers like the Rust healthz worker never receive a stray trace-context argument
